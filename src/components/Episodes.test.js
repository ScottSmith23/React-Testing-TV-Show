...
test ("renders",()=> {
  ...
  test("renders episodes", () => {
    ...
    const { queryByTestId, rerender, debug, getAllByTestId } = render(
      <Episodes episodes={[]}  />
    );  
    ...
    expect(queryByTestId(/episode/i)).toBeNull();  // rerender the component with new props (errors)
    rerender(<Episodes episodes={mockEpisodesData}/>);  // assert that the error message is rendered
    expect(getAllByTestId(/episode-list/i)).toHaveLength(3);
  });